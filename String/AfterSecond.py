...
def after_second(target_string, search_string):
    first_index = target_string.find(search_string)
    second_index = target_string.find(search_string, first_index+len(search_string), len(target_string)-1)
    return target_string[second_index+len(search_string):]
