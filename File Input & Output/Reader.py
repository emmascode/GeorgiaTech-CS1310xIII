...
def reader(file_name):
    ...
    for item in aList:
        item = item.split()
        bList.append(item)
    for my_string in bList:
        for item in my_string:
            my_string[0] = int(my_string[0])
            my_string[2] = int(my_string[2])
            my_string[3] = int(my_string[3])
            my_string[4] = float(my_string[4])  
    file.close()
    
    for my_string in bList:
        res.append(tuple(my_string))
    return res 
